...
for (const t of TARGETS) {
  const name = `@lgcode/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "lgcode.exe" : "lgcode";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `lgcode prebuilt binary for ${t.node}.`,
...
const mainDir = join(STAGE, "lgcode");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "lgcode", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "lgcode", "package.json"), "utf8"),
);
...
// stable to default with a manual `npm dist-tag add lgcode@<ver> latest`.
...
console.log(`publish lgcode@${version} (${distTag})`);
