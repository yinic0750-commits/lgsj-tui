# LGcode Desktop (Wails shell)

A native desktop window around the LGcode Go kernel. The same
...
```sh
cd desktop
wails build          # → build/bin/LGcode(.app/.exe)
```
...
- **macOS** — open `LGcode-darwin-universal.dmg` and drag LGcode into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/LGcode.app
  ```
...
```sh
minisign -Vm LGcode-darwin-arm64.zip \
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `LGCODE_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
