...
import (
	...
	"lgcode/desktop/internal/update"
)
...
func TestSignFiles(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "LGcode-linux-amd64.tar.gz")
...
func TestGenManifest(t *testing.T) {
	dir := t.TempDir()
	names := []string{
		"LGcode-darwin-arm64.zip",
		"LGcode-darwin-amd64.zip",
		"LGcode-windows-amd64-installer.exe",
		"LGcode-windows-amd64.zip", // portable download, not the updater channel
		"LGcode-windows-arm64-installer.exe",
		"LGcode-windows-arm64.zip", // portable download, not the updater channel
		"LGcode-linux-amd64.tar.gz",
		"LGcode-linux-amd64.deb",            // human download, not the updater channel
		"LGcode-linux-amd64.tar.gz.minisig", // must be skipped
	...
	t.Setenv("GITHUB_REPOSITORY", "esengine/lgcode")
	...
	wantURL := "https://github.com/esengine/lgcode/releases/download/desktop-v1.2.0/LGcode-windows-amd64-installer.exe"
	if win.URL != wantURL {
	...
	if !strings.HasSuffix(arm.URL, "/LGcode-windows-arm64-installer.exe") {
	...
	if !strings.HasSuffix(lin.URL, "/LGcode-linux-amd64.tar.gz") {
