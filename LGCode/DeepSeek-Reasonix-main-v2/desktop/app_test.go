...
api_key_env = "DEEPSEEK_API_KEY"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "lgcode.toml"), []byte(projectConfig), 0o644); err != nil {
		t.Fatalf("write project config: %v", err)
	}
...
func TestDeleteProviderMigratesConfigAndOpenTabs(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("LGCODE_TEST_KEY", "sk-test")

	cfg := config.Default()
	cfg.DefaultModel = "prov-a/model-a2"
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", Models: []string{"model-a1", "model-a2"}, APIKeyEnv: "LGCODE_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "LGCODE_TEST_KEY"},
	}
...
func TestDeleteProviderRejectsRunningAffectedTab(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", APIKeyEnv: "LGCODE_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "LGCODE_TEST_KEY"},
	}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)
...
func TestSubmitToTabHistoryDisplaysRawInputAfterMemoryCompose(t *testing.T) {
	...
	ctrl.QueueMemory(`Saved memory "lgcode-contributions": contribution count updated`)
...
func TestForkCreatesActiveTabWithoutSwitchingSourceController(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workspace, "lgcode.toml"), []byte("[codegraph]\nenabled = false\n"), 0o644); err != nil {
...
func TestCapabilitiesShowsDefaultMCPAsInitializingNotDisabled(t *testing.T) {
	...
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "lgcode.toml"), []byte(`
[codegraph]
enabled = false
...
`), 0o644); err != nil {
	...
	projectCfg := config.LoadForEdit(filepath.Join(dir, "lgcode.toml"))
...
`), 0o644); err != nil {
	...
	if err := app.UpdateMCPServer("broken", MCPServerInput{
		Name:      "broken",
		Transport: "stdio",
		Command:   "lgcode-missing-mcp-binary",
	}); err != nil {
	...
	if got := cfg.Plugins[0].Command; got != "lgcode-missing-mcp-binary" {
	...
	for _, s := range view.Servers {
		if s.Name == "broken" {
			...
			if s.Command != "lgcode-missing-mcp-binary" || s.Tier != "background" {
...
func TestSetMCPServerTierEnablesCodegraphAndIgnoresLegacyTier(t *testing.T) {
	...
	t.Setenv("LGCODE_CACHE_DIR", robustTempDir(t)) // isolate the codegraph bundle cache so Resolve fails deterministically
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "lgcode.toml"), []byte(`
[codegraph]
...
func TestBuiltInMCPBackgroundNotifyDoesNotDownload(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("LGCODE_CACHE_DIR", robustTempDir(t))
