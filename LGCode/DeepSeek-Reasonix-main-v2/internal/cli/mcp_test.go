...
func TestRenderMCPManagerDetailCompactsConfigPath(t *testing.T) {
	p := &mcpManager{
		...
		name:  "github",
		snapshot: mcpSnapshot{
			configPath: "/Users/example/very-long/Library/Application Support/lgcode/config.toml",
			servers: []mcpServerView{{
	...
	if strings.Contains(got, "Application Support/lgcode/config.toml") {
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/lgcode.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/lgcode.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("lgcode.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "lgcode.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("lgcode.toml")
	if err != nil {
...
func TestApplyMCPModeRecordsPluginConnectFailure(t *testing.T) {
	...
	t.Setenv("PATH", "")
	cfg := config.Default()
	cfg.Plugins = []config.PluginEntry{{Name: "broken", Command: "definitely-missing-lgcode-mcp", Tier: "lazy"}}
	if err := cfg.SaveTo("lgcode.toml"); err != nil {
		t.Fatalf("save config: %v", err)
	}
...
func TestApplyMCPModeRecordsCodegraphConnectFailure(t *testing.T) {
	isolateUserConfig(t)
	t.Setenv("PATH", "")
	t.Setenv("LGCODE_CACHE_DIR", t.TempDir())
	cfg := config.Default()
