...
func TestVerifyChecksum(t *testing.T) {
	...
	t.Run("match", func(t *testing.T) {
		checksumFile := []byte(fmt.Sprintf("%s  lgcode-linux-amd64.tar.gz\n", hash))
		if err := verifyChecksum(content, "lgcode-linux-amd64.tar.gz", checksumFile); err != nil {
	...
	t.Run("not found", func(t *testing.T) {
		checksumFile := []byte(fmt.Sprintf("%s  lgcode-darwin-arm64.tar.gz\n", hash))
		if err := verifyChecksum(content, "lgcode-linux-amd64.tar.gz", checksumFile); err == nil {
...
func TestExtractFromTarGz(t *testing.T) {
	// Build a .tar.gz in memory containing a "lgcode" entry.
	...
	got, err := extractFromTarGz(buf.Bytes(), "lgcode")
	if err != nil {
...
func TestExtractFromTarGz_Nested(t *testing.T) {
	...
	if err := tw.WriteHeader(&tar.Header{
		Name: "lgcode-linux-amd64/lgcode",
		Mode: 0o755,
		Size: int64(len(body)),
	}); err != nil {
		t.Fatal(err)
...
func TestExtractFromTarGz_NotFound(t *testing.T) {
	...
	_, err := extractFromTarGz(buf.Bytes(), "lgcode")
	if err == nil {
...
func TestPickCLIRelease(t *testing.T) {
	...
	// selected, not skipped — `lgcode upgrade` always moves to the newest 1.x.
	withRC := []ghRelease{
		{TagName: "v1.7.0-rc.1"},
		{TagName: "v1.6.0"},
	}
