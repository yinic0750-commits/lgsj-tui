...
// upgradeCommand handles `lgcode upgrade` (and `lgcode update`).
func upgradeCommand(args []string, version string) int {
	...
	base := fmt.Sprintf("lgcode-%s-%s", runtime.GOOS, runtime.GOARCH)
	var asset *ghAsset
...
// "npm-v"). Prereleases are kept: only 1.x carries `lgcode upgrade`, and the
...
func fetchLatestRelease(c *http.Client) (*ghRelease, error) {
	...
	req.Header.Set("User-Agent", "lgcode-cli")
...
// extractBinary pulls the "lgcode" binary from a .tar.gz or .zip archive.
...
// rename it aside to .lgcode.old first, then place the new binary.
