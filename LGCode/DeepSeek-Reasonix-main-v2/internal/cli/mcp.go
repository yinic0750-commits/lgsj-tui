...
// mcp.go holds the MCP server-management surface shared by the `lgcode mcp`
...
// mcpCommand implements `lgcode mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `lgcode codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: lgcode mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to lgcode.toml).

Usage:
  lgcode mcp list
  lgcode mcp add <name> <command> [args...]        stdio server
  lgcode mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  lgcode mcp add <name> --sse  <url>               remote (legacy SSE)
  lgcode mcp import                                import Codex-enabled servers from cc-switch
  lgcode mcp remove <name>
...
Examples:
  lgcode mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  lgcode mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
