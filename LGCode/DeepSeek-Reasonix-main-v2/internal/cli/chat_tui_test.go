...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"lgcode chat", "› /mcp"}
	...
	if !strings.Contains(lines[0], "lgcode chat") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "lgcode.toml")
