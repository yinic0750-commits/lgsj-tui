...
func RunCommand(args []string, in io.Reader, out io.Writer, errOut io.Writer, version string) int {
	if len(args) != 1 || args[0] != TimeName {
		fmt.Fprintln(errOut, "usage: lgcode builtin-mcp time")
		return 2
	}
...
// ServeTimeMCP serves LGcode's dependency-free built-in time MCP over stdio.
...
func handleTimeLine(line []byte, w *bufio.Writer, version string) error {
	...
	case "initialize":
		resp.Result = map[string]any{
			...
			"serverInfo": map[string]any{"name": "lgcode-time", "version": version},
		}
	case "tools/list":
