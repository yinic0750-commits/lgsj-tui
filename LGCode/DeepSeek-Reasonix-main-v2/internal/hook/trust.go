...
// Trust gates project hooks. A project's .lgcode/settings.json can run
...
// root. The trust flag lives in user-global state (~/.lgcode/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.lgcode/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.lgcode.
...
// TrustPath is ~/.lgcode/trust.json (homeDir overrides ~).
