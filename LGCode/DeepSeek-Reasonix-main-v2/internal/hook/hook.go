...
// (.lgcode/settings.json, only when the project is trusted) and a global
// (~/.lgcode/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".lgcode"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.lgcode/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.lgcode/settings.json.
