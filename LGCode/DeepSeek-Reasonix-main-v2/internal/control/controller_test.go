...
func TestRemoveMCPServerRemovesUnconnectedLazyPlaceholder(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	if err := os.WriteFile("lgcode.toml", []byte(`
[[plugins]]
name = "mock"
command = "mock-mcp"
tier = "lazy"
...
func TestApprovalPersistentBashPrefixRememberRule(t *testing.T) {
	...
	c := New(Options{
		...
		OnRemember: func(rule string) RememberResult {
			remembered = rule
			return RememberResult{Rule: rule, Path: "lgcode.toml", Saved: true}
		},
	})
	go func() {
	...
	if len(notices) != 1 || !strings.Contains(notices[0], "Bash(go test:*)") || !strings.Contains(notices[0], "lgcode.toml") {
