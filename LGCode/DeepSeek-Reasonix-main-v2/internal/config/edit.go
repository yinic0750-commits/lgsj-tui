...
// separate from the `lgcode setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to LGCODE_LANG / locale.
...
// .mcp.json-only server would otherwise be serialized into lgcode.toml or the
...
func pluginTOMLSourcePath(name string) string {
	for _, path := range []string{"lgcode.toml", userConfigPath()} {
...
// half-written lgcode.toml that fails to parse on next load. Parent directories
...
func SaveMinimalProjectAutoPlan(path, mode string) (string, error) {
	...
	body := fmt.Sprintf(`# LGcode project configuration.
...
func writeConfigFile(path, body string) error {
	...
	tmp, err := os.CreateTemp(dir, ".lgcode.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./lgcode.toml when none exists yet — the conventional
...
// SaveForRoot saves the config to root's lgcode.toml, falling back to the
// user's global config when root has no existing lgcode.toml.
