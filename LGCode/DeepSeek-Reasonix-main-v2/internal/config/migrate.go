...
// legacyConfig is the subset of the v0.x (~/.lgcode/config.json) schema this
// import carries forward. Fields absent here are dropped on purpose: desktop tab
// state is frontend-owned, and skills already live in the shared ~/.lgcode/skills
...
func (r *MigrationResult) Notice() string {
	...
	if r.KeyToEnv {
		b.WriteString("; API key saved to lgcode's credentials store")
	}
...
// checks v1-era TOML first, then v0.5/v0.x ~/.lgcode/config.json, and never
...
func MigrateLegacyIfNeeded() (*MigrationResult, error) {
	...
	src := filepath.Join(home, ".lgcode", "config.json")
	...
	if qqSecret := strings.TrimSpace(legacy.QQ.AppSecret); qqSecret != "" {
		...
		res.Warnings = append(res.Warnings, "your previous QQ Bot App Secret was saved to lgcode's credentials store")
	}
...
func legacyTOMLPaths(dest, home string) []string {
	paths := []string{filepath.Join(filepath.Dir(dest), "lgcode.toml")}
	if home != "" {
		paths = append(paths, filepath.Join(home, ".lgcode", "lgcode.toml"))
	}
	return paths
}
...
// writeCredentialsEnv merges lines into the lgcode-owned global credentials
// file (UserCredentialsPath, e.g. %AppData%\lgcode\credentials), replacing any
