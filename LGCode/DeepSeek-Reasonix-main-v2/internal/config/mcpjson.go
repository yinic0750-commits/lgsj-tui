...
// mcpJSONFile is the project-root file Claude Code calls .mcp.json. LGcode reads
...
// legacyConfigPath is the v0.x (TypeScript line) config file, ~/.lgcode/config.json.
func legacyConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".lgcode", "config.json")
}

// loadLegacyMCP reads the v0.x ~/.lgcode/config.json and returns its enabled
...
// already declare. lgcode.toml's [[plugins]] win on a name collision: it is the
// LGcode-specific, more explicit of the two, so it overrides the shared,
