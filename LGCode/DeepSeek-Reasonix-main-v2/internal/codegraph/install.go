...
const (
	...
	officialMirrorBase         = "https://dl.lgcode.io/codegraph"
...
// <user cache>/lgcode/codegraph/<Version>. Versioned so a bump installs cleanly
// beside the old one. LGCODE_CACHE_DIR overrides the base (relocate the cache,
...
func cacheRoot() string {
	base := os.Getenv("LGCODE_CACHE_DIR")
	if base == "" {
		var err error
		...
		base = filepath.Join(base, "lgcode")
	}
	return base
}
...
// verifying it against the checksum baked into the lgcode binary, then returns
// the launcher path.
...
// InstallWithClient is Install with an explicit HTTP client, used when LGcode
...
func installVersionWithClient(ctx context.Context, client *http.Client, version, want string, bases []string, force bool, log func(string)) (string, error) {
	...
	if err := promote(root, dir); err != nil {
		...
		return "", fmt.Errorf("codegraph: install to %s failed: %w — the cache directory may be read-only or locked by antivirus; set LGCODE_CACHE_DIR to a writable location to relocate it", dir, err)
	}
