...
// than shipped in the lgcode binary, which keeps installs small. Resolve finds
...
// LGcodeDaemonIdleTimeoutMS keeps LGcode-launched daemons short-lived after
// the last client disconnects while still preserving CodeGraph's shared-client
// refcount semantics.
const LGcodeDaemonIdleTimeoutMS = "1000"
...
// BundleDirName is the optional directory, beside the lgcode executable, where
...
// bundled looks for the CodeGraph launcher unpacked beside the lgcode binary.
...
// normal LGcode controller close must only close its proxy connection and let
