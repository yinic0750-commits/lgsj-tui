...
// buildExamplePlugin compiles cmd/lgcode-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "lgcode-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "lgcode/cmd/lgcode-plugin-example").CombinedOutput()
	if err != nil {
