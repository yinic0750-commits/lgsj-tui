...
var ChineseTraditional = Messages{
	...
	SetKeyHint:      "設定好 API key 後執行 `lgcode chat`。",
	ConfigLabel:     "設定",
	ModelsLabel:     "模型",
	...
	InitHint:       "專案記憶（AGENTS.md）在會話內由模型生成：執行 `lgcode chat`，然後 `/init` —— 模型會分析程式碼庫並寫入。設定請用 `lgcode setup`。",
	StepSetKeyHint: "執行 `lgcode setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "互動式會話",
	StepRunDesc:    "執行單次任務",
	HelpFooter:     "lgcode help · 檢視全部命令",

	ChatTip:           "對話上下文將跨輪保留。輸入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "沒有可恢復的會話 — 用 `lgcode chat` 開一個新的",
	...
	OutputStyleHint:    "在 lgcode.toml 設定 agent.output_style 即可啟用（下次會話生效）",
	ThemeHeader:        "主題：",
	...
	SlashMCPNone:                 "沒有設定 MCP 伺服器 — 在 lgcode.toml 加一個 [[plugins]] 條目",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "當前",
	ArgLanguageAuto: "從 LGCODE_LANG / 系統 locale 自動偵測",
	...
	ListHooksNone:       "無生效 hooks — 在 .lgcode/settings.json（專案，需信任後）或 ~/.lgcode/settings.json（全域）設定",
	ListMcpHeader:       "MCP 伺服器",
	ListMcpNone:         "未連線 MCP 伺服器 — 在 lgcode.toml（[[plugins]]）或專案 .mcp.json 中新增",

	MemoryNone:             "還沒有載入任何記憶 — 輸入 “/remember 內容” 可快速記錄，也可以在專案根目錄建立 LGCODE.md",
	...
	MissingKeyIntro:       "lgcode.toml 已設定好 — 只差一個 API key 就可以開始。",
	WroteFileFmt:          "已寫入 %s",
	SetupComplete:         "設定完成。",
	SetupCancelled:        "設定已取消。",
	TryHintFmt:            "試試: %s",
	NextHint:              "下一步：設定 API key（執行 `lgcode setup` 或 export DEEPSEEK_API_KEY=...），然後執行 `lgcode run \"你的任務\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳過 lgcode.toml 裡的舊 %q 條目（指向 %s）— 請手動從 [[providers]] 裡刪除",
	...
	ProviderErrAuth:                "認證失敗 (HTTP 401)：API key 缺失、錯誤或已過期。請檢查 .env 中的金鑰，或執行 `lgcode setup`。",
	...
	UsageBody: `lgcode — 由設定和插件驅動的 coding agent（多模型）

用法：
  lgcode chat [--model NAME] [-c|--continue] [--resume]   互動式會話（多輪；-c 恢復最近一次，--resume 選擇一個）
  lgcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   執行單次任務後退出
  lgcode serve [--model NAME] [--addr HOST:PORT]      透過 HTTP+SSE 提供會話（瀏覽器客戶端在 /）
  lgcode acp [--model NAME]                           透過 stdio 提供 Agent Client Protocol（也可用：lgcode --acp）
  lgcode setup [path]                                 互動式設定精靈；生成 lgcode.toml（及 .env）
  lgcode config auto-plan [off|on]                    設定自動計畫模式
  lgcode config reasoning-language [auto|zh|en]        設定可見思考語言
  lgcode mcp <add|remove|list>                        管理 lgcode.toml 裡的 MCP 伺服器
  lgcode doctor [--json]                              輸出脫敏的本機診斷資訊
  lgcode version
  lgcode help
...
範例：
  ...
  lgcode run "把 main.go 裡的 TODO 實現掉"
  lgcode run --model mimo-pro "給這個函式補單元測試"
  echo "解釋這段程式碼" | lgcode run

設定：
  優先順序：flag > ./lgcode.toml > ~/.config/lgcode/config.toml > 內建預設值
  金鑰透過 api_key_env 從環境變數注入（如 DEEPSEEK_API_KEY）。
  執行 'lgcode setup' 生成設定；詳見 docs/SPEC.md。
`,
	...
	GoalSetFmt:                 "目標已設定 → %s",
	GoalCleared:                "目標已清除",
	ProviderErrAuthRejected:    "認證失敗 (HTTP 401)：服務端拒絕了你的 API key。可能是 key 錯誤或已過期，也可能是服務端出現瞬時鑑權/額度問題——已退避重試仍失敗。請稍後再試，或檢查 .env 中的金鑰 / 執行 `lgcode setup`。",
