...
// $LC_ALL / $LGCODE_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that LGCODE_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("LGCODE_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("LGCODE_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("LGCODE_LANG=en overriding LANG=zh: got %q, want en", got)
	}
