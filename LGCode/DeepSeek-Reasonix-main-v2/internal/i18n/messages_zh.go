...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `lgcode chat`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	ListHooksNone:       "无生效 hooks — 在 .lgcode/settings.json（项目，需信任后）或 ~/.lgcode/settings.json（全局）配置",
	...
	MissingKeyIntro:       "lgcode.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（运行 `lgcode setup` 或 export DEEPSEEK_API_KEY=...），然后运行 `lgcode run \"你的任务\"`。",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API key（缺失或未设置）。请在 .env 中配置密钥，或运行 `lgcode setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `lgcode setup`。",
	...
	UsageBody: `lgcode — 由配置和插件驱动的 coding agent（多模型）
