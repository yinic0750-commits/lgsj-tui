...
Hooks 让 LGcode 在会话、用户输入、工具调用、模型返回、压缩上下文等节点执行本地 shell 命令。桌面端在“设置 -> Hooks”里提供图形化编辑入口，本质上读写同一份 `settings.json`。
...
```json
{
  "hooks": {
    ...
    "Stop": [
      {
        "command": "echo LGcode turn finished"
      }
    ]
  }
}
...
```json
{
  "hooks": {
    ...
    "Stop": [
      { "command": "osascript -e 'display notification \"Turn done\" with title \"LGcode\"'" }
    ]
  }
}
```
...
`command` 会通过平台 shell 执行：macOS/Linux 使用 `sh -c`，Windows 使用 `cmd /c`。stdin 是 LGcode 写入的一行 JSON，见下面的 payload 表。
...
LGcode 会把一行 JSON 写入 hook 命令的 stdin。所有 payload 都至少有：
...
- `PostLLMCall`：exit 0 且 stdout 非空时，stdout 会替换用户看到的 reasoning。若 provider 的 reasoning 带签名，LGcode 会保留原始 signed reasoning 用于后续请求，同时仍展示 hook 转换后的文本。
