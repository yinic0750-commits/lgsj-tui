# Migrating to LGcode 1.0 (the Go rewrite)

LGcode 1.0 is a **ground-up rewrite in Go**. It is a new codebase, not an
incremental upgrade of the `0.x` TypeScript releases. This guide explains what
changed and how to move over.

## TL;DR

| | Legacy (v1) | LGcode 1.0+ (v2) |
|---|---|---|
| Language | TypeScript / Node | Go |
| Branch | [`v1`](https://github.com/esengine/DeepSeek-LGcode/tree/v1) (maintenance only) | `main-v2` (default, active) |
...
```sh
git clone https://github.com/esengine/DeepSeek-LGcode   # default: main-v2 (Go)
cd DeepSeek-LGcode && make build                        # -> bin/reasonix(.exe)
```
...
| project memory | `LGCODE.md` (+ auto-memory), Claude-Code-compatible |
...
LGcode 1.0 supports reading and editing files in UTF-8, UTF-8 BOM, UTF-16
...
Questions? Open a [Discussion](https://github.com/esengine/DeepSeek-LGcode/discussions).
