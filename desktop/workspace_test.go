...
func TestDialogDefaultDirectoryUsesFileParent(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "lgcode.toml")
...
func TestReadFileMediaPreview(t *testing.T) {
	...
	if !strings.HasPrefix(image.URL, "/__lgcode_workspace_media/") || !strings.HasSuffix(image.URL, "/shot.PNG") {
	...
	if !strings.HasPrefix(pdf.URL, "/__lgcode_workspace_media/") || !strings.HasSuffix(pdf.URL, "/report.pdf") {
...
func TestMediaTokenHandlerBadToken(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/__lgcode_workspace_media/deadbeef/fake.png", nil)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
