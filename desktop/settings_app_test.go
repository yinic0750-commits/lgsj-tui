...
func TestSetReasoningLanguageUpdatesLiveTabControllers(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(projectRoot, "lgcode.toml"), []byte("[agent]\nreasoning_language = \"en\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestProjectHooksSettingsUseActiveWorkspaceRootAndTrust(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(project, ".lgcode", "settings.json")); err != nil {
...
func TestSaveHooksSettingsForRootUsesDisplayedProjectRoot(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(projectA, ".lgcode", "settings.json")); err != nil {
		t.Fatalf("displayed project root settings missing: %v", err)
	}
	if _, err := os.Stat(filepath.Join(projectB, ".lgcode", "settings.json")); err == nil {
