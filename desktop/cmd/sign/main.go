...
// by substring (file names embed the key, e.g. LGcode-darwin-arm64.zip), so the
...
// (lgcode.key) and the public key (lgcode.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "lgcode.key")
	pubPath := filepath.Join(dir, "lgcode.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "LGcode desktop release")
		out := f + ".minisig"
