...
function makeMockApp(): AppBindings {
  ...
  const globalWorkspaceRoot = "~/Library/Application Support/lgcode/global-workspace";
  let cwd = freshMock ? globalWorkspaceRoot : "~/projects/joyquant-db"; // mutable so PickWorkspace is visible in dev
  let workspaces = freshMock ? [] : ["~/projects/joyquant-db", "~/projects/joyquant-sys", "~/projects/lgcode", "~/projects/blade"];
  let mockEffort = "auto";
  ...
  const capSkills: SkillView[] = [
    ...
    { name: "init", description: "Scaffold a LGCODE.md for this repo", scope: "builtin", runAs: "inline", enabled: true },
  ];
  let capSkillRoots: SkillRootView[] = [
    { dir: "~/projects/lgcode/.lgcode/skills", scope: "project", priority: 1, status: "missing", configured: false, removable: true, skills: 0 },
    {
      dir: "~/my-skills",
      scope: "custom",
      priority: 5,
    ...
    },
    {
      ...
      skillItems: [
        ...
        { name: "init", description: "Scaffold a LGCODE.md for this repo", scope: "global", runAs: "inline" },
      ],
    },
  ];
  ...
  const settings: SettingsView = {
    ...
    agent: { temperature: 0.2, maxSteps: 0, plannerMaxSteps: 12, systemPrompt: "You are LGcode, a coding agent.", coldResumePrune: true, reasoningLanguage: "auto" },
    bot: {
      enabled: !freshMock,
      model: "",
    ...
    telemetry: true,
    metrics: false,
    configPath: "~/projects/lgcode/lgcode.toml",
  ...
  if (freshMock) {
    settings.configPath = "~/.config/lgcode/config.toml";
  }
  ...
  return {
    ...
    async PickWorkspace() {
      ...
      return mockSwitchWorkspace(cwd.endsWith("another-project") ? "~/projects/lgcode" : "~/projects/another-project");
    },
      ...
      builtInMCPUpdates = [
        {
          ...
          latest: "v0.10.0",
          phase: "activated",
          path: "/tmp/lgcode/codegraph/mock/codegraph",
        },
      ];
      return { name, version: "v0.10.0", path: "/tmp/lgcode/codegraph/mock/codegraph" };
    },
    ...
    async SavePastedImage(_dataUrl: string) {
      return ".lgcode/attachments/mock.png";
    },
    async SaveClipboardImage() {
      return ".lgcode/attachments/mock-clipboard.png";
    },
    async SavePastedFile(name: string, _dataUrl: string) {
      return `.lgcode/attachments/mock-${name}`;
    },
    ...
    async AttachDropped(path: string) {
      ...
      return { kind: "attachment" as const, path: `.lgcode/attachments/mock-${name}` };
    },
    ...
    async Memory() {
      return {
        available: true,
        storeDir: "~/.config/lgcode/projects/-mock/memory",
        storeGlobalDir: "~/.config/lgcode/memory/global",
        docs: [
          {
            path: "LGCODE.md",
            scope: "project",
            body: "# LGcode project memory\n\nMock doc shown in the browser dev seam.\n\n## Notes\n\n- prefers concise replies",
          },
          {
            path: "~/.config/lgcode/LGCODE.md",
            scope: "user",
        ...
        archives: [
          {
            ...
            path: "~/.config/lgcode/projects/-mock/memory/.archive/20260612-021500.000-old-plan.md",
            archivedAt: "2026-06-12T02:15:00Z",
          },
        ],
        scopes: [
          { scope: "user", path: "~/.config/lgcode/LGCODE.md" },
          { scope: "project", path: "LGCODE.md" },
          { scope: "local", path: "LGCODE.local.md" },
        ],
      };
    },
    async MemorySuggestions() {
      return {
        ...
        skills: [
          {
            id: "skill-lgcode-pr-followup",
            name: "lgcode-pr-followup",
            description: "Review or update a LGcode GitHub PR, address feedback, verify, and publish safely.",
            scope: "project",
            body: "# LGcode PR Followup\n\nUse this skill for repeated LGcode PR work.\n\n## Workflow\n\n1. Confirm branch and PR state.\n2. Inspect the diff.\n3. Fix actionable feedback.\n4. Verify and update the PR.\n",
    ...
    async AcceptSkillSuggestion(suggestion: SkillSuggestion) {
      ...
      return `.lgcode/skills/${suggestion.name}/SKILL.md`;
    },
    ...
    async Remember(_scope: string, _note: string) {
      ...
      return `${_scope} LGCODE.md (mock): ${_note}`;
    },
      ...
      settings.permissions[k] = settings.permissions[k].filter((r) => r !== rule);
        ...
        async StartBotConnectionInstall(provider: string, domain: string) {
          ...
          return {
            ...
            url: "https://example.com/lgcode-bot-qr",
    ...
    async OpenDownloadPage() {
      if (typeof window !== "undefined") {
        window.open("https://github.com/esengine/lgcode/releases/latest", "_blank", "noopener");
      }
    },
