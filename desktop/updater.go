...
const (
	r2Base         = "https://dl.lgcode.io"
	ghReleasesBase = "https://github.com/esengine/lgcode/releases"
...
func applyLinux(targz []byte) error {
	bin, err := extractBinary(targz, "lgcode-desktop")
	if err != nil {
		return err
	}
...
func applyWindows(installer []byte) error {
	f, err := os.CreateTemp("", "lgcode-update-*.exe")
	if err != nil {
		return err
	}
	name := f.Name()
