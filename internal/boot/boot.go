...
type Options struct {
	...
	// controller but are not persisted to lgcode.toml.
...
func Build(ctx context.Context, opts Options) (*control.Controller, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w %q (configured: %s); note: defining [[providers]] replaces the built-in presets, so add a [[providers]] entry for it or use a configured name, or run `lgcode setup` to reconfigure", ErrUnknownModel, modelName, providerNames(cfg))
	}
	...
	if migErr != nil {
		sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "config migration from ~/.lgcode failed: " + migErr.Error()})
	...
	// Persistent memory (LGCODE.md / AGENTS.md hierarchy + auto-memory index)
	...
	if cfg.Codegraph.Enabled && !tokenEconomy {
		...
		default:
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelInfo,
				Text: "codegraph: not installed — run `lgcode codegraph install` to enable symbol-graph tools"})
		}
	}
	if !tokenEconomy {
	...
	// resolves "ask" to allow — preserving `lgcode run` autonomy — while deny
	...
	skillRunner := func(sctx context.Context, sk skill.Skill, task string, runOpts skill.SubagentRunOptions) (string, error) {
		...
		if subagentStore == nil || parentSession == "" {
			// Headless runs (e.g. `lgcode run`) have no persistent session to
	...
	// Custom slash commands (.lgcode/commands + user dir). Best-effort: a malformed
...
func migrateLegacySessionSources(sink event.Sink) {
	...
	if home, herr := os.UserHomeDir(); herr == nil {
		sources = append(sources, legacySource{
			dir:     filepath.Join(home, ".lgcode", "sessions"),
			label:   "~/.lgcode/sessions",
...
func rememberPermissionConfigPath(workspaceRoot string) string {
	...
	if workspaceRoot != "" {
		return filepath.Join(workspaceRoot, "lgcode.toml")
	}
	path := config.SourcePath()
	if path == "" {
		path = "lgcode.toml" // match Config.Save() fallback
	}
	return path
}
