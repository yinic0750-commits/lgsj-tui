...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — lgcode setup
	StepSetKey      string // step 2 command label

	// `lgcode init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // lgcode chat step desc
	StepRunDesc    string // lgcode run step desc
	HelpFooter     string // dim footer linking to lgcode help
	...
	WroteFileFmt          string // "Wrote %s" — used for lgcode.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from lgcode.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: lgcode run [--model NAME] <task>"
	...
	// `lgcode upgrade` / `lgcode update` — self-update
...
//
// Priority: override > LGCODE_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"LGCODE_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
