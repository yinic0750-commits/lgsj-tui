...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `lgcode chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold lgcode.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `lgcode chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `lgcode setup`.",
	StepSetKeyHint: "run `lgcode setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "lgcode help · all commands",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `lgcode chat`",
	...
	ChatStatusCycleHint:         "shift+tab 切换计划 · ctrl+y 自动审批",
	...
	OutputStyleHint:    "set agent.output_style in lgcode.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in lgcode.toml",
	...
	ArgLanguageAuto: "auto-detect from LGCODE_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .lgcode/settings.json (project, after trust) or ~/.lgcode/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in lgcode.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create LGCODE.md in the project root",
	...
	MissingKeyIntro:       "lgcode.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `lgcode setup` or export DEEPSEEK_API_KEY=...), then run `lgcode run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from lgcode.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: lgcode run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `lgcode setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `lgcode setup`.",
	...
	UsageBody: `lgcode — a config- and plugin-driven coding agent (multi-model)

Usage:
  lgcode chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  lgcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  lgcode serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  lgcode acp [--model NAME]                           serve Agent Client Protocol over stdio (also: lgcode --acp)
  lgcode setup [path]                                 interactive config wizard; writes lgcode.toml (+ .env)
  lgcode config auto-plan [off|on]                    configure automatic plan mode
  lgcode config reasoning-language [auto|zh|en]        configure visible reasoning language
  lgcode mcp <add|remove|list>                        manage MCP servers in lgcode.toml
  lgcode doctor [--json]                              print redacted local diagnostics
  lgcode bot start|doctor|weixin-login                multi-channel IM bot gateway
  lgcode upgrade [--check] [--force]                   self-update to the latest release
  lgcode version
  lgcode help

Examples:
  lgcode chat
  lgcode chat --continue
  lgcode run "implement the TODOs in main.go"
  lgcode run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | lgcode run

Configuration:
  Resolution: flag > ./lgcode.toml > ~/.config/lgcode/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'lgcode setup' to scaffold a config; see docs/SPEC.md.
`,
}
