...
var Chinese = Messages{
	...
	Ready:           "已就绪",
	GetStarted:      "开始使用",
	StepScaffold:    "生成 lgcode.toml",
	StepSetKey:      "设置 API 密钥",

	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `lgcode chat`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `lgcode setup`。",
	StepSetKeyHint: "运行 `lgcode setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "lgcode help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `lgcode chat` 开一个新的",
	...
	OutputStyleHint:    "在 lgcode.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashHelp:          "命令：/compact · /new · /clear · /resume · /rewind · /tree · /branch · /switch · /todo · /verbose · /model（切换模型）· /effort · /theme · /language · /mcp · /技能 · /钩子 · /paste-image · /memory · /goal · /remember · /quit · /help · 以及 技能（/init、/explore …）",
	...
	SkillPickerSourceHint:        "↑↓ 移动 · Enter 查看技能 · d 诊断 · s 技能 · r 重新扫描 · Esc 关闭",
	...
	SkillPickerNoChanges:         "技能对话框已关闭：没有更改",
	SkillPickerSourceSkillsHint:  "↑↓ 移动 · Space 开关 · Enter 详情 · Esc 返回来源",
	SkillPickerSourceSkillsEmpty: "这个来源下没有技能",
	...
	SkillPickerTokenFmt:          "~%d 词元",
	SkillPickerDetailMetaFmt:     "范围: %s  运行方式: %s",
	SkillPickerSkillsUnit:        "个技能",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 lgcode.toml 加一个 [[plugins]] 条目",
	...
	ArgSkillShow:    "查看技能内容",
	ArgSkillNew:     "新建一个技能",
	ArgSkillPaths:   "显示技能发现路径",
	ArgMcpAdd:       "连接一个服务器",
	ArgMcpRemove:    "断开一个服务器",
	ArgMcpList:      "显示已配置的服务器",
	ArgMcpConnected: "已连接",
	ArgHooksList:    "列出生效的 钩子",
	ArgHooksTrust:   "信任本项目的 钩子",
	ArgModelCurrent: "当前",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "当前",
	ArgLanguageAuto: "从 LGCODE_LANG / 系统 locale 自动检测",
	ArgLanguageEn:   "English",
	ArgLanguageZh:   "中文",

	ListModelsHeaderFmt: "模型（当前：%s）",
	ListModelsHint:      "用底部的模型切换器，或输入 /model <供应商/模型>",
	...
	ListSkillsHeaderFmt: "技能（%d 个）",
	ListSkillsNone:      "暂无技能 — 调用内置的（如 /init），或用 install_skill 创建一个",
	ListHooksHeaderFmt:  "钩子（生效 %d 个）",
	ListHooksNone:       "无生效 钩子 — 在 .lgcode/settings.json（项目，需信任后）或 ~/.lgcode/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 lgcode.toml（[[plugins]]）或项目 .mcp.json 中添加",

	MemoryNone:             "还没有加载任何记忆 — 输入 “/remember 内容” 可快速记录，也可以在项目根目录创建 LGCODE.md",
	...
	ModelListHeader:        "模型（/model <供应商/模型> 切换）",
	RewindNone:             "暂无可回滚的内容",
	...
	SelectProvidersLabel:  "选择要启用的供应商",
	EnterAPIKeysHeader:    "输入 API 密钥（回车跳过、稍后再设）：",
	MissingKeyIntro:       "lgcode.toml 已配置好 — 只差一个 API 密钥就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API 密钥（运行 `lgcode setup` 或 export DEEPSEEK_API_KEY=...），然后运行 `lgcode run \"你的任务\"`。",
	...
	FamilyKeyPromptFmt:         "输入 %s 的 API 密钥以获取可用模型（回车跳过）：",
	...
	SkipStaleCustomEntryFmt:    "跳过 lgcode.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	CustomPromptKeyEnv:   "请输入 API 密钥环境变量名",
	CustomPromptAPIKey:   "请输入 API 密钥",
	...
	AnthropicPromptKeyEnv:          "请输入 API 密钥环境变量名",
	AnthropicPromptAPIKey:          "请输入 API 密钥",
	...
	UsageRunHint:              "用法：lgcode run [--model NAME] <task>",
	ErrorPrefix:               "错误：",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API 密钥（缺失或未设置）。请在 .env 中配置密钥，或运行 `lgcode setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API 密钥。可能是密钥错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `lgcode setup`。",
	...
	UsageBody: `lgcode — 由配置和插件驱动的 编程助手（多模型）

用法：
  lgcode chat [--model NAME] [-c|--continue] [--resume]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个）
  lgcode run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   执行单次任务后退出
  lgcode serve [--model NAME] [--addr HOST:PORT]      通过 HTTP+SSE 提供会话（浏览器客户端在 /）
  lgcode acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：lgcode --acp）
  lgcode setup [path]                                 交互式配置向导；生成 lgcode.toml（及 .env）
  lgcode config auto-plan [off|on]                    配置自动计划模式
  lgcode config reasoning-language [auto|zh|en]        配置可见思考语言
  lgcode mcp <add|remove|list>                        管理 lgcode.toml 里的 MCP 服务器
  lgcode doctor [--json]                              输出脱敏的本地诊断信息
  lgcode bot start|doctor|weixin-login                多渠道 IM bot 网关
  lgcode upgrade [--check] [--force]                   自更新到最新版本
  lgcode version
  lgcode help
...
示例：
  ...
  lgcode run "把 main.go 里的 TODO 实现掉"
  lgcode run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | lgcode run

配置：
  优先级：flag > ./lgcode.toml > ~/.config/lgcode/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入（如 DEEPSEEK_API_KEY）。
  运行 'lgcode setup' 生成配置；详见 docs/SPEC.md。
`,
}
