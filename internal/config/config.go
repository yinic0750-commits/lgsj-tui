// Package config loads LGcode's runtime configuration from TOML. Resolution order:
// flag > project ./lgcode.toml > user config.toml (in the OS user-config dir) > built-in defaults.
...
// Config is LGcode's runtime configuration.
type Config struct {
	ConfigVersion     int                     `toml:"config_version"`
	DefaultModel      string                  `toml:"default_model"`
	Language          string                  `toml:"language"` // ui/model language tag (e.g. "zh"); empty = auto-detect from $LANG / $LGCODE_LANG
	UI                UIConfig                `toml:"ui"`
...
// lets lgcode fetch the CodeGraph runtime into its cache when CodeGraph is
// enabled but missing; set false to require an explicit `lgcode codegraph
...
// BuiltInMCPConfig controls LGcode-shipped MCP servers that require no user
...
// BuiltInMCPUpdatesConfig controls background update checks for LGcode-owned
...
// the project roots (.lgcode/.agents/.agent/.claude under the workspace) and
...
type AgentConfig struct {
	...
	// .lgcode/output-styles/<name>.md). Empty = the unmodified prompt.
...
// lgcode.toml's [[plugins]] or a project-root .mcp.json (see loadMCPJSON).
...
const DefaultSystemPrompt = `You are LGcode, a coding agent focused on executing code tasks.
...
func Default() *Config {
	return &Config{
		...
		// Mode "ask" with no rules keeps `lgcode run` autonomous (no TTY → ask
		// resolves to allow) while `lgcode chat` prompts before writers. Users add
...
// the v0.x ~/.lgcode/config.json's mcpServers. A .env in the working directory
...
// each project's lgcode.toml + .env + .mcp.json are resolved independently
...
func LoadForRoot(root string) (*Config, error) {
	...
	projectTOML := "lgcode.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "lgcode.toml")
	}
	...
	// project lgcode.toml doesn't drop the global config's MCP servers.
	...
	// lgcode.toml wins on a name collision (see mergeMCPJSON).
	mcpFile := mcpJSONFile
	if root != "." {
	...
	// Lowest priority: the v0.x ~/.lgcode/config.json's mcpServers, so upgrading
...
// LoadForEdit returns a config to seed the `lgcode setup` wizard when reconfiguring:
...
func userConfigPath() string {
	...
	return filepath.Join(dir, "lgcode", "config.toml")
}
...
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/lgcode/config.toml"
	}
...
// UserCredentialsPath is the lgcode-owned global secrets file, beside
// config.toml in the user config dir (os.UserConfigDir()/lgcode/credentials). It
...
func UserCredentialsPath() string {
	...
	return filepath.Join(dir, "lgcode", "credentials")
}
...
func ArchiveDir() string {
	...
	return filepath.Join(dir, "lgcode", "archive")
}

// SessionDir is where chat sessions are persisted (one .jsonl per session).
// Used by `lgcode chat --continue` / `--resume` to find the recent ones. Empty
...
func SessionDir() string {
	...
	return filepath.Join(dir, "lgcode", "sessions")
}
...
// existing dirs (UserConfigDir/lgcode/...) so the whole lgcode state tree
...
func CacheDir() string {
	...
	return filepath.Join(dir, "lgcode", "cache")
}

// MemoryUserDir returns the lgcode user config root (…/lgcode), under which
// the user-global LGCODE.md and the per-project auto-memory store live. Empty
...
func MemoryUserDir() string {
	...
	return filepath.Join(dir, "lgcode")
}

// ConventionDirs are the parent directories scanned for agent assets (skills,
// commands), in canonical-first order. .lgcode is ours; .agents / .agent /
...
// .lgcode/settings.json (see internal/hook).
var ConventionDirs = []string{".lgcode", ".agents", ".agent", ".claude"}

// conventionSubdirsAsc joins sub under each ConventionDir of base, in ascending
// priority (reverse of ConventionDirs) so the canonical .lgcode ends up the
...
// on a name clash. Order: home-dir convention dirs (~/.claude/commands … ~/.lgcode/commands),
// the legacy XDG user dir (~/.config/lgcode/commands), then the project's
// convention dirs (.claude/commands … .lgcode/commands). Scanning the .claude /
...
func CommandDirsForRoot(root string) []string {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		dirs = append(dirs, filepath.Join(dir, "lgcode", "commands")) // legacy XDG user dir
	}
