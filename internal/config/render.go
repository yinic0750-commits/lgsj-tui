...
// RenderTOML renders the config as annotated TOML in the `lgcode setup` house style:
...
// lgcode.toml stays focused on project behavior and intentionally excludes
...
func RenderTOMLForScope(c *Config, scope RenderScope) string {
	...
	b.WriteString("# LGcode configuration.\n")
	fmt.Fprintf(&b, "# Resolution order: flag > ./lgcode.toml > %s > built-in defaults.\n", userConfigDisplayPath())
	...
	if c.Language != "" {
		fmt.Fprintf(&b, "language      = %q   # ui/model language; empty = auto-detect from $LANG / $LGCODE_LANG\n", c.Language)
	} else {
		b.WriteString("# language      = \"zh\"   # ui/model language; empty = auto-detect from $LANG / $LGCODE_LANG\n")
	}
	b.WriteString("\n")

	if shouldRenderUI(c, defaults, scope) {
		b.WriteString("[ui]\n")
		fmt.Fprintf(&b, "theme = %q   # auto|dark|light; CLI colors only; LGCODE_THEME can override per run\n", c.UITheme())
		if style := c.UIThemeStyle(); style != "" {
			fmt.Fprintf(&b, "theme_style = %q   # CLI accent palette; LGCODE_THEME_STYLE can override per run\n", style)
		} else {
	...
	if shouldRenderNetwork(c, defaults, scope) {
		...
		if c.Network.Proxy.Password != "" {
			...
			b.WriteString("# password = \"${LGCODE_PROXY_PASSWORD}\"   # optional; supports ${VAR} expansion\n")
		}
		b.WriteString("\n")
	}
	...
	if c.Codegraph.Path != "" {
		...
		b.WriteString("# path       = \"\"   # empty = cache, then PATH, then a bundle beside lgcode\n")
	}
	b.WriteString("\n")
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"lgcode-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `lgcode setup`
