...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "lgcode-plugin-example"}); err != nil {
...
func TestLoadForEditPreservesCodegraphWithoutSection(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "lgcode.toml")
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "lgcode.toml")
...
func TestSaveToScopesUserAndProjectFiles(t *testing.T) {
	...
	projectPath := filepath.Join(t.TempDir(), "lgcode.toml")
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(root, "lgcode.toml"), []byte(`
[[providers]]
name = "deepseek-flash"
kind = "openai"
base_url = "https://api.deepseek.com"
...
`), 0o644); err != nil {
	...
	projectPath := filepath.Join(root, "lgcode.toml")
