...
func TestUserConfigDisplayPathCollapsesHome(t *testing.T) {
	...
	if !strings.HasSuffix(got, "lgcode/config.toml") {
		t.Fatalf("display path = %q, want lgcode/config.toml suffix", got)
	}
...
func TestRenderTOMLRoundTrips(t *testing.T) {
	...
	orig.Network = NetworkConfig{
		...
		Proxy: NetworkProxyConfig{
			...
			Port:     7890,
			Username: "user",
			Password: "${LGCODE_PROXY_PASSWORD}",
		},
	}
	...
	orig.Plugins = []PluginEntry{
		{Name: "example", Command: "lgcode-plugin-example"},
	...
	if len(got.Bot.Connections) != 1 || got.Bot.Connections[0].Model != "deepseek-pro" || got.Bot.Connections[0].WorkspaceRoot != "/tmp/lgcode-bot" {
	...
	if len(got.Bot.Connections[0].SessionMappings) != 1 || got.Bot.Connections[0].SessionMappings[0].Scope != "project" || got.Bot.Connections[0].SessionMappings[0].WorkspaceRoot != "/tmp/lgcode-bot" {
...
func TestRenderTOMLDefaultStepsDoNotOverrideGlobalConfig(t *testing.T) {
	home := isolateUserConfigHome(t)
	globalDir := filepath.Join(home, ".config", "lgcode")
	...
	projectPath := filepath.Join(projectDir, "lgcode.toml")
