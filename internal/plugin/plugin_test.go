...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "lgcode-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "lgcode-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "lgcode-missing-mcp-binary" not found on PATH`,
