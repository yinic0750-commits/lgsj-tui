// Package memory implements LGcode's persistent memory. It mirrors Claude
// Code's two-layer model while honoring LGcode's cache-first architecture:
//
//   - Hierarchical doc memory: LGCODE.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/lgcode/LGCODE.md
	ScopeAncestor Scope = "ancestor" // a LGCODE.md above the project root
	ScopeProject  Scope = "project"  // ./LGCODE.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./LGCODE.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// LGCODE.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"LGCODE.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"LGCODE.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
