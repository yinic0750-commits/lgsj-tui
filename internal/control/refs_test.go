...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".lgcode/attachments/clipboard-20260601-010203.000000.png": true,
		".lgcode/attachments/clipboard-20260601-010203.000000.yml": true,
		".lgcode/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".lgcode/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".lgcode/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".lgcode/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".lgcode", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".lgcode/attachments/config.yml")
	zipRef := filepath.ToSlash(".lgcode/attachments/archive.zip")
	pngRef := filepath.ToSlash(".lgcode/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: lgcode\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: lgcode") {
...
func TestWorkspaceImageRefsOnlyTreatAttachmentsAsImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".lgcode", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".lgcode/attachments/shot.png" {
