...
func TestConfigureCLIThemeSwitchesModeAndDefaultStyle(t *testing.T) {
	...
	t.Setenv("LGCODE_THEME", "")
	t.Setenv("LGCODE_THEME_STYLE", "")
	...
	if got := accent("x"); !strings.HasPrefix(got, "\033[38;5;34m") {
		t.Fatalf("light default accent = %q, want sandstone xterm 34", got)
	}
...
func TestConfigureCLIThemeStyleOverride(t *testing.T) {
	...
	if got := accent("x"); !strings.HasPrefix(got, "\033[38;5;120m") {
		t.Fatalf("aurora accent = %q, want xterm 120", got)
	}
...
func TestConfigureCLIThemeHonorsEnvOverride(t *testing.T) {
	...
	if activeCLITheme.name != "dark" || activeCLITheme.style != "ember" {
		t.Fatalf("LGCODE_THEME override resolved %s/%s, want dark/ember", activeCLITheme.name, activeCLITheme.style)
	}
}
