...
func TestGitStatusRenderWithinCompactsRepoBeforeBranch(t *testing.T) {
	status := gitStatus{Repo: "VeryLongDeepSeekLGcodeWorkspace", Branch: "codex/cli-tui-status-row"}

	full := ansi.Strip(status.RenderWithin(80, statusAutoColor))
	if full != "VeryLongDeepSeekLGcodeWorkspace@codex/cli-tui-status-row" {
...
func TestGitStatusRenderWithinKeepsDirtySuffix(t *testing.T) {
	status := gitStatus{
		Repo:      "VeryLongDeepSeekLGcodeWorkspace",
...
func TestLoadGitStatus(t *testing.T) {
	...
	runGitForTest(t, root, "config", "user.email", "lgcode@example.invalid")
	runGitForTest(t, root, "config", "user.name", "LGcode Test")
