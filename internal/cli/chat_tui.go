...
type chatTUI struct {
	...
	// commands are custom slash commands loaded from .lgcode/commands; each renders
...
// greenPixelLogo returns the embedded ANSI pixel-art LGcode branch logo.
func greenPixelLogo() string {
	return greenPixelLogoAns
}

func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	logoLines := strings.Split(greenPixelLogo(), "\n")
	logoW := 0
	for _, l := range logoLines {
		if w := visibleWidth(l); w > logoW {
			logoW = w
		}
	}
	...
	if missing != "" {
		headerLines = append(headerLines, "", yellow("! "+missing))
	}

	maxLines := len(logoLines)
	if len(headerLines) > maxLines {
		maxLines = len(headerLines)
	}
	for i := 0; i < maxLines; i++ {
		...
		b.WriteString(logo)
		b.WriteString(strings.Repeat(" ", logoW-visibleWidth(logo)+2))
		b.WriteString(head)
		b.WriteByte('\n')
	}
	b.WriteString("\n" + dim("  "+i18n.M.ChatTip) + "\n")
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.lgcode/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)
