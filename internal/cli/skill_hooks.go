...
func (m *chatTUI) skillList() {
	...
	if len(skills) == 0 {
		m.notice("no skills found. Add SKILL.md / <name>.md under .lgcode/skills (project) or ~/.lgcode/skills (global); .agents/.agent/.claude skills dirs also work. Invoke with /<name> or run_skill.")
		return
	}
...
func (m *chatTUI) runHooksSubcommand(input string) {
	...
	case "trust":
		...
		m.notice("trusted this project's hooks — restart LGcode to load them")
	default:
