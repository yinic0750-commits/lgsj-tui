...
func TestACPInitializesWithoutAPIKey(t *testing.T) {
	...
	if !strings.Contains(out, `"protocolVersion":1`) || !strings.Contains(out, `"name":"lgcode"`) {
...
func TestACPFactoryLoadsSessionCwdProjectConfig(t *testing.T) {
	...
	t.Setenv("LGCODE_TEST_KEY", "test-key")
	project := t.TempDir()
...
model = "fake-model"
api_key_env = "LGCODE_TEST_KEY"
`), 0o644); err != nil {
		t.Fatal(err)
	}
	cmdDir := filepath.Join(project, ".lgcode", "commands")
