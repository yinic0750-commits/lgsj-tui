// Package cli implements lgcode's command-line entry: subcommand routing, flag
...
func Run(args []string, version string) int {
	...
	case "init":
		...
		// for config), so `lgcode init` isn't a dead end.
		configureCLIThemeFromConfigNoProbe()
		return initHint()
	case "acp":
		configureCLIThemeFromConfigNoProbe()
...
func runServe(args []string) int {
	...
	fmt.Printf("lgcode serve — %s on http://%s\n", ctrl.Label(), *addr)
...
// Keys always go to the lgcode-owned global credentials file so they never land
...
// project-local lgcode.toml only when the user config dir can't be resolved.
...
// defaultEnvTarget is the lgcode-owned global credentials file, falling back to
...
// resolveSetupTargets picks where `lgcode setup` writes. Keys always go to the
// global env. The config goes to the user-global dir by default, to ./lgcode.toml
...
// setupConfig runs the configuration wizard (the `lgcode setup` command),
// writing config.toml to the user-global dir (or ./lgcode.toml under --local)
// and API keys to the lgcode-owned global .env — never a project's own .env.
...
// initHint handles `lgcode init`. Unlike a config scaffold, project memory is
...
// (and to `lgcode setup` for config) so the verb isn't a dead end.
func initHint() int {
...
// and any entered API keys to envPath (the lgcode-owned global .env, never a
...
func interactiveSetup(configPath, envPath string) int {
	...
	fmt.Print(boxed([]string{
		accent("◆") + " " + fmt.Sprintf(i18n.M.WelcomeTitleFmt, bold("lgcode")),
		"",
...
// that older versions of the wizard wrote into lgcode.toml. They collide
...
// can warn the user to clean up lgcode.toml by hand.
...
// duplicate provider entries on subsequent re-runs of `lgcode setup`.
// The host-based slug also gives users a meaningful name to grep for in
// lgcode.toml. Falls back to a short sha1 of the raw URL when the URL
...
// not exist in config because users editing lgcode.toml deal with SKU names
...
// them — a lgcode.toml that defines only [[providers]] for deepseek otherwise
// hides the other presets from setup, since [[providers]] replaces the presets
// wholesale. Families
...
// appended to the lgcode-owned global .env so the chat session that follows
...
// are appended — so re-running `lgcode setup` with a corrected key replaces the
...
func welcome(version string) int {
	...
	b.WriteString(boxed([]string{
		accent("◆") + " " + bold("lgcode") + "  " + dim(version),
		dim(i18n.M.Subtitle),
	}))
	...
	if src == "" {
		step("lgcode setup", i18n.M.StepScaffold)
	}
	if ready == 0 {
		step(i18n.M.StepSetKey, i18n.M.StepSetKeyHint)
	}
	step("lgcode chat", i18n.M.StepChatDesc)
	step(`lgcode run "task"`, i18n.M.StepRunDesc)
...
func configAutoPlanCommand(args []string) int {
	...
	local := fs.Bool("local", false, "write ./lgcode.toml instead of the user config")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	rest := fs.Args()
...
func configUsage() {
	fmt.Print(`Usage:
  lgcode config auto-plan [--local] [off|on]
  lgcode config reasoning-language [--local] [auto|zh|en]
`)
}
