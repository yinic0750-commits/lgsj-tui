...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.lgcode/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.lgcode/skills/ (this workspace only); 'global' writes to ~/.lgcode/skills/ (every project)."
	}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Identifier — letters/digits/_/-/., 1-64 chars, starts alphanumeric. Becomes the skill folder name under ~/.lgcode/skills/<name>/SKILL.md."},
