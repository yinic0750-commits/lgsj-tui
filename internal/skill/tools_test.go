...
func TestRunSkillInline(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".lgcode/skills/note.md", "---\ndescription: take a note\n---\nDo the thing.")
...
func TestRunSkillSubagentNeedsRunner(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".lgcode/skills/dig.md", "---\ndescription: dig\nrunAs: subagent\n---\nbody")
...
func TestRunSkillSubagentResolvesProfile(t *testing.T) {
	home := t.TempDir()
	writeSkill(t, home, ".lgcode/skills/deep.md", "---\ndescription: deep\nrunAs: subagent\nmodel: deepseek-pro\neffort: max\n---\nbody")
	tl := NewRunSkillTool(New(Options{HomeDir: home, DisableBuiltins: true}), nil)
...
func TestInstallSkill(t *testing.T) {
	...
	wantPath := filepath.Join(home, ".lgcode", "skills", "deploy", SkillFile)
	...
	if _, err := os.Stat(filepath.Join(home, ".lgcode", "skills", "deploy.md")); !errors.Is(err, os.ErrNotExist) {
