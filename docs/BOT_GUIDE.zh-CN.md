...
> 里使用 LGcode，以及审批、问答、YOLO 和常用命令的交互方式。
...
连接机器人后，你可以在飞书、Lark 或微信里给 LGcode 发消息，让桌面端
LGcode 在本机执行同一套模型、工具、权限与沙盒逻辑。

典型场景：

- 让 LGcode 查代码、读文档、解释错误、整理结论。
...
打开桌面端 LGcode，进入 **设置 -> 机器人**。在 **添加 IM Bot** 区域选择
渠道并扫码。
...
不能替代用户白名单。远端用户进入的是同一个 LGcode controller、权限策略、
...
LGcode 的机器人沿用桌面端权限系统。默认是询问模式：写文件、执行命令等
敏感工具调用会先请求确认。
...
正常升级或覆盖安装 LGcode app 后，不需要重新绑定。

绑定信息保存在用户配置目录，而不是 app 包内：

- Bot 连接、远端 ID、白名单、模型和审批模式保存在用户配置文件。
- 飞书和 Lark 的密钥保存在 LGcode 的用户凭据文件。
- 微信扫码后的账号 token 保存在 LGcode 的用户数据目录。
...
| 已连接但没有回复 | 确认桌面端 LGcode 正在运行，Bot 连接已开启，用户 ID 在白名单内或允许所有人。 |
