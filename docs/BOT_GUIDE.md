...
> bots, how to use LGcode from IM, and how approvals, Ask questions, YOLO, and
> bot commands work.
...
After a bot is connected, you can send LGcode messages from Feishu, Lark, or
...
- Ask LGcode to inspect code, read docs, explain errors, or summarize findings.
...
Open the LGcode desktop app and go to **Settings -> Bots**. In **Add IM Bot**,
...
- `credential.app_id`, `credential.app_secret_env`, `credential.account_id`,
  ...
  tokens. Secrets stay in environment variables or the LGcode user credentials
  store.
...
sequenceDiagram
  ...
  participant R as "LGcode desktop"
...
LGcode bots use the same permission system as the desktop app. Ask mode is the
...
No. A normal LGcode app upgrade or overwrite install does not require
rebinding.

Bindings are stored in the user's LGcode data, not inside the app bundle:

- Bot connections, remote IDs, allowlists, model choices, and approval modes are
  stored in the user config.
- Feishu and Lark secrets are stored in the LGcode user credentials file.
- The WeChat scanned account token is stored in the LGcode user data
  directory.

You may need to bind again if:

- The LGcode user config directory was deleted.
...
| Connected but no reply | Make sure the LGcode desktop app is running, the bot connection is enabled, and the sender ID is allowlisted or access is open. |
