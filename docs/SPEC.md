# LGcode Engineering Spec

> LGcode is a coding agent: a thin harness driving multiple models, with **all
...
Long tasks eventually fill the model's context window. LGcode manages this with
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — LGcode
...
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $LGCODE_LANG

[agent]
system_prompt = "You are LGcode, a coding agent..."  # or system_prompt_file = "..."
...
LGcode-specific source). This lets a server already configured for Claude work in
LGcode unchanged.
