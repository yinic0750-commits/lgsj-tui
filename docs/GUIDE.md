...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $LGCODE_LANG
...
new key combination, and LGcode saves it for the desktop app. Conflicting
...
LGcode is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
**Already have an `.mcp.json`?** Drop it in the project root and LGcode
...
`/memory` lists both memory documents (`LGCODE.md` / `AGENTS.md`) and saved
...
Embed `@` references in a message and LGcode resolves them before sending, as
...
The planner sees loaded `LGCODE.md` / `AGENTS.md` memory and a small read-only
...
automatically: LGcode first drafts a read-only plan, then waits for approval
