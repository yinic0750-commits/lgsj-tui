...
# language    = "zh"               # 界面语言；为空则按 $LANG / $LGCODE_LANG 自动检测
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，LGcode 会为桌面端保存该绑定。
...
LGcode 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，LGcode 会原样读取——其
...
`/memory` 会同时列出记忆文档（`LGCODE.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
在消息里写 `@` 引用，LGcode 会在发送前解析成带标签的上下文块：`@path/to/file`（或
...
Planner 会看到已加载的 `LGCODE.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
的任务会自动进入 plan mode：LGcode 先只读生成计划，待用户批准后才
